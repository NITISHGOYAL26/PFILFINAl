...
echo " 
    <body>
		...
		echo " <div class='panel panel-$theme'>
                        <div class='panel-heading'> View My Staff </div> 
...
if(isset($_REQUEST['id']))
{
	...
	$id=$_REQUEST['id'];
	echo "<a href='viewemployee.php?pageno=$pageno'>Back</a><br/>";	

$newsql="SELECT `id`,`employee_Name`,`Higher_qualification`,`marital`,`gender`,`email`,`contact_number`,`dateofbirth`,`address`,`resume_path`,`image_path` FROM `emp1` WHERE id=$id";
//echo $newsql;
...
else
{
			...
			$sql="SELECT `id`,`employee_Name`,`Higher_Qualification`,`gender`,`email`,`contact_number` FROM `emp1` limit $offset,$size";
			$query= mysqli_query($con,$sql);
			$num= mysqli_num_rows($query);
			$num_of_col= mysqli_num_fields($query);
			
			echo "Total No Employee:$num";
			...
			for($c=0;$c<$num_of_col;$c++)
			echo "<td>$row[$c]</td>";
			echo"<td><a href='viewemployee.php?id=$row[0]&pageno=$pageno' class='btn btn-$theme'>View all info</a></td>"; 
			echo "</tr>";
			$s++;
			}
		echo "</table>";
		...
		{
			$lastpageno=$pageno-1;
		echo "<a href='viewemployee.php?pageno=$lastpageno'><<</a> ";
		}
		...
		for($j=1,$i=$j-1;$i<$total_pages;$i++,$j++)
		{
			...
			echo "<a href='viewemployee.php?pageno=$i'>$j</a> &nbsp;";
		}
		if($pageno<$total_pages-1)
		{$pageno++;
		echo "<a href='viewemployee.php?pageno=$pageno'>>></a>";
		}
